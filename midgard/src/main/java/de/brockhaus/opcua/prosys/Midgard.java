...
public class Midgard 
{
	...
	public void doRead() throws ServiceException, StatusException, ServiceResultException, InterruptedException
	{
		...
		// select the MyObjects node & browse the references for this node
		nodeId = selectNode(0);
		browse(nodeId);
		
		// select the Simulation node & browse its references
		nodeId = selectNode(4);
		browse(nodeId);
		
		// select the Sawtooth1 node & browse its references
		nodeId = selectNode(5);
		browse(nodeId);
